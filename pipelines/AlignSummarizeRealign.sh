...
then
  bowtie2 --very-sensitive-local --un-conc-gz ${dir}/${sample}-unmapped \
	-X 2300 \
	-x ${reference} \
	-1 ${dir}/${sampleshort}-filtered.1.fastq.gz \
	-2 ${dir}/${sampleshort}-filtered.2.fastq.gz \
	-S ${dir}/${sample}.sam \
...
if [ ! -f ${dir}/${sample}.summary ] || [ ${clean} -eq "0" ];
then
...
if [ ! -f ${dir}/${sample}-realigned.bam ] || [ ${clean} -eq "0" ];
then
...
if [ ! -f ${dir}/${sample}-clean.bam ] || [ ${clean} -eq "0" ];
then
